...
const SlidersManagement: React.FC = () => {
  ...
  const fetchSliders = async () => {
    try {
      setLoading(true);
      const response = await apiClient.getSliders();
      setSliders(response);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('=== FRESH SLIDER MANAGEMENT DEBUG ===');
    ...
    try {
      ...
      let response;
      if (editingSlider) {
        response = await apiClient.updateSlider(editingSlider._id, formDataToSend);
        setSuccess('Slider updated successfully!');
      } else {
        response = await apiClient.createSlider(formDataToSend);
      ...
      console.error('Error submitting slider:', err);
  ...
  const handleToggleActive = async (id: string, isActive: boolean) => {
    try {
      await apiClient.toggleSliderStatus(id, !isActive);
      setSuccess(`Slider ${!isActive ? 'activated' : 'deactivated'} successfully!`);
      fetchSliders();
    } catch (error) {
  ...
  const handleSetDefault = async (id: string) => {
    try {
      await apiClient.setDefaultSlider(id);
      setSuccess('Default slider updated successfully!');
      fetchSliders();
    } catch (error) {
      console.error('Error setting default slider:', error);
      setError('Failed to set default slider');
    }
  };
